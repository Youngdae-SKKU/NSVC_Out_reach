...
// 📅 D-Day를 자동으로 계산해 주는 함수입니다 (8월 5일 기준)
function getDDay() {
  const now = new Date();
  const target = new Date('2026-08-05T00:00:00+09:00'); // 2026년 8월 5일 자정 (한국시간)
...
function getNoticeStyle(note: string, title: string) {
  if (note?.includes('중요') || title.includes('TEST')) return { icon: '📢', color: 'text-rose-600', bg: 'bg-rose-50' };
  if (note?.includes('기도') || title.includes('기도') || title.includes('Pray')) return { icon: '🙏', color: 'text-blue-600', bg: 'bg-blue-50' };
      ...
      <div className="relative z-10 bg-gradient-to-r from-[#8B5CF6] via-[#6366F1] to-[#3B82F6] pt-10 pb-8 text-center rounded-b-[30px] shadow-lg">
        <div className="max-w-6xl mx-auto px-4">
          ...
          <div className="inline-flex items-center gap-2 bg-white/20 text-white text-[12px] md:text-[14px] font-black pl-4 pr-1.5 py-1.5 rounded-full backdrop-blur-md border border-white/30 mb-3 tracking-widest shadow-sm">
            ...
            {/* 눈에 확 띄는 노란색 캡슐 + 빛나는 그림자 + 깜빡임(pulse) 애니메이션 */}
        ...
        <div className="flex flex-col gap-3">
          ...
          <Link href="/intro" className="relative rounded-[1.5rem] overflow-hidden shadow-xl group h-[160px] flex flex-col justify-between p-5 border border-white/40 hover:-translate-y-1 transition-transform duration-300">
            ...
            <div className="relative z-10 text-white">
              ...
              <p className="text-xs md:text-sm mt-1 text-gray-200 font-medium truncate drop-shadow-sm">
                저희와 함께 새로운 믿음의 여정을 떠나실래요?
              </p>
            </div>
            ...
            <div className="relative z-10 flex justify-between items-end mt-auto">
              ...
              <span className="bg-white text-slate-900 font-black text-[13px] px-4 py-2 rounded-full shadow-lg group-hover:bg-slate-100 transition-colors">
                자세히 보기 &rarr;
              </span>
            </div>
          </Link>
            ...
            <div className="flex-1 space-y-2 relative z-10">
              {recentNotices.length > 0 ? (
                recentNotices.map((notice: any) => {
                  ...
                  return (
                    <div key={notice.id} className="flex items-start p-2.5 bg-white rounded-xl shadow-sm border border-slate-100 hover:border-indigo-200 hover:shadow-md transition-all cursor-pointer group">
                      <div className={`w-9 h-9 shrink-0 rounded-lg flex items-center justify-center text-lg ${style.bg} shadow-inner mt-0.5`}>
                        {style.icon}
                      </div>
                      <div className="flex flex-col flex-1 ml-3 overflow-hidden">
                        <span className={`text-[9px] font-black ${style.color} mb-0.5`}>{notice.note || "소식"}</span>
                        <h4 className="text-sm font-black text-slate-800 group-hover:text-indigo-600 transition-colors leading-snug break-keep">
                          {notice.title}
                        </h4>
                        <span className="text-[9px] text-slate-400 font-medium mt-1">📅 {notice.date}</span>
                      </div>
                    </div>
                  );
                })
